...
- **App.xaml / App.xaml.cs + AssemblyInfo.cs** — 入口。允许多实例并行运行（无单实例限制；两实例共用同一 Config 文件，后保存者生效）；Config `SoftwareRender=1` 时 `RenderOptions.ProcessRenderMode=SoftwareOnly`（GPU 驱动黑屏机器的逃生开关）；`DispatcherUnhandledException` + `AppDomain.UnhandledException` 兜底写入 `%AppData%\TrFileTransfer\logs\crash.log` 并弹友好提示；`App.ExePath` 供 AutoStart/Updater/防火墙提示取 exe 路径；`ShutdownMode=OnMainWindowClose`。程序集属性（版本 2.7.0.0，`Updater.CurrentVersion` 读取）在 `AssemblyInfo.cs`（csproj `GenerateAssemblyInfo=false` 避免与源内 attribute 冲突）。
