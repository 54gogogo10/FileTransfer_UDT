...
测试套件包含单元测试（L10N、Config、Shared 工具方法、ServerResumeStore、FolderResumeStore、Updater 清单解析/版本比较/SHA256/Apply 回滚）和 29 项集成测试（TCP：单文件/文件夹/空文件/中文名/多客户端/连接拒绝/文件不存在/大文件×2/续传×4/限速；UDT：单文件/文件夹/空文件/限速/大文件×2/续传×3；自动升级：检查+下载校验/哈希不匹配/清单404/清单无效/旧版本——经 `MiniHttpServer`（TcpListener 手写 HTTP，无 HttpListener URL ACL 依赖）提供本地清单与文件；另有双协议发现测试）。使用自定义轻量级 `TestRunner` + `Assert` 类，无外部测试框架依赖。
