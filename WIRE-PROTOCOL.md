...
### 加密认证（类型 `0x07`，可选前导帧，v2.9+）

客户端填有配对码且开启"加密传输"时，用 0x07 取代 0x05：认证与密钥协商一次完成，成功后整个会话（头部、文件内容、哈希、文本——全部字节）经加密流传输。旧版本服务器不认识 0x07（当作 0x00 头解析必然校验失败并断开），客户端据此回退为明文 0x05 重连一次。

```
[1 byte:  0x07]
[32 bytes: SHA256(UTF-8 配对码)]   // 与 0x05 相同的预共享码哈希
...
密钥派生（两端一致，纯 SHA256 级联，无外部依赖）：

```
master  = SHA256(salt ‖ psk)                    // psk = SHA256(配对码)
...
加密流（`EncryptedWireStream`，每个方向独立）：上层字节切成 ≤256 KB 的段，每段在线上为：

```
[4 bytes: Int32 plainLen（小端）]
[16 bytes: 截断的 HMAC-SHA256(macKey, lenBytes ‖ ciphertext)]
...
无独立响应帧——送达确认与文件语义一致：TCP 以连接干净关闭为准，UDT 复用既有的 1 字节应用层 ACK。接收端经 `OnTextReceived` 事件上抛给 UI（日志 + 气泡通知 + 非模态文本窗口）。
